...
# for i in range(20):
    try:
        resp = requests.get(url.format(start_num - i), headers=headers)
        ...
        if content_list[0] == "0":
            print("可以下载，序号：{}".format(start_num - i))
            list_data.append([start_num - i, None])
        else:
            print("无法下载，序号：{}，金币：{}".format(start_num - i, content_list[0]))
            continue
    except:
        print("出现异常，序号：{}".format(start_num - i))

    if i % 20 == 0:
        print("存储20的倍数")
        dict_data = {"down_list": list_data}
        write_json_file("./jiaoxue.json", dict_data)
